...
- **Fixed PR #16's failing Vercel build** — two independent causes:
  1. The root `tsconfig.json` `include: ["**/*.ts(x)"]` swept `mobile/**` into the website's type check, and React Native's global `FormData` type conflicts with lib.dom's (surfaced in `upload/route.ts`). Fix: `"exclude": ["node_modules", "mobile"]` — the Expo app type-checks with its own tsconfig.
  2. **Every Vercel preview build was failing** (since S22 at latest): all env vars were Production-scope only, and the S22 routes create service-role clients at module scope, so page-data collection died with "supabaseUrl is required." Fixed by adding the `.env.local` values to the **Preview** environment via `vercel env add <name> preview` (all except `CRON_SECRET`, which only production cron needs). Note: sensitive Vercel env values cannot be pulled back out (`vercel env pull` returns them empty). Code-hygiene follow-up candidate: make the module-scope admin clients lazy so builds don't depend on env at all.
...
- **Mobile create-listing flow** (second PR, S24 continued) — `mobile/app/listing/new.tsx`, mirroring the website's 5-section design: searchable country selector with state dropdown and prefill from the user's latest listing, `[H]`/`[W]` title inputs (40 chars) with live preview, Haves/Wants sections with a new **RN `CardPicker`** (`mobile/components/CardPicker.tsx` — a port of the web CardPicker: same fetch plan/sort/filter logic, TCGdex direct + pokemontcg.io image fallback, EN/JP toggle, era/set dropdowns, type pills, debounced search), want-pref chips, description (300). Submits via Bearer `POST /api/listings` (no Turnstile, B2 path), redirects to the new listing's detail screen. Entry point: yellow "+ Post" button in the Browse tab header. `mobile/lib/cardData.ts` + `cardData.generated.json` are **copies** of the website module (no workspace, D10) — re-copy after `npm run update-cards`.
- **React Compiler gotcha (important for all future mobile code):** the app has `experiments.reactCompiler` enabled, and the compiler hoists member expressions from non-null assertions (e.g. `country!.name` inside a callback) into eagerly-evaluated memo deps — crashing the whole screen while the value is still null. Use plain `if (!x) return;` guards inside callbacks instead of `!.`.

**Next (M1):** realtime chat screen (inbox conversation detail), my-listings, push-notification registration, forgot-password, in-app account deletion + block (M1-8 compliance).
